...
# text directory selector (m: monolingual, p: parallel)
mode = ""

def getMode(path):
    """ Pre: file structure is thus that directory containing parallel or
    monolingual text files is marked as such. "parallel" or "monolingual"
    must be the end of the directory name and separated by "_".
...
def writeJSON(imagePath, textPath):
    """ Appends JSON Lines file with image and text path pair to be fed to prodigy 
    as stream for annotation.
    """
    #JSON format
    dict = {"image": imagePath, "text": textPath}
    #append file
...
def main():
    ...
    corresponding text file path, appends the pair to the specified .jsonl
    file..
    """
    ...
    for subdirectory0 in os.listdir(imageDirectory):
        ...
        for subdirectory1 in os.listdir(imageDirectory 
            ...
            for filename in os.listdir(imageDirectory 
                                       + subdirectory0 
                                       + subdirectory1):
                imagePath = subdirectory1 + filename
                ...
                textPath = textDirectory + mode + "/" + corpusFileNr[0] + ".txt"
